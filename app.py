...
def pcm_to_wav(pcm, sample_rate=24000):
    ...
    header = struct.pack(
        ...
        b"RIFF", 36 + data_size,
        b"WAVE",
        b"fmt ", 16, 1, nc, sample_rate,
        sample_rate * nc * (bps // 8),
        nc * (bps // 8), bps,
        b"data", data_size,
    )
    return header + pcm
...
def concat_close_transcripts(segments, threshold=2.0, max_chunk_sec=15):
    """
    BUG FIX: আগে MAX_DUR=120s + factor=2 → সব segments 1-এ merge হয়ে "1/1 done"।
    এখন max 15s, threshold doubling নাই। Default-এ OFF (worker check করে)।
    Enhancement: TTS-কে natural pause নিত বলার জন্য merge-এ "(pause for N seconds)" text inject করে (zip reference থেকে)।
    """
    if not segments:
        return []
...
async def _gemini_tts_call(text, voice, api_key, language="Bengali", model=None):
    ...
    async with httpx.AsyncClient(timeout=90) as client:
        ...
        b64 = (resp.json().get("candidates", [{}])[0]
...
def pick_edge_voice(gemini_voice, user_choice="auto"):
    if user_choice and user_choice != "auto":
        return user_choice
    if gemini_voice in GEMINI_MALE_VOICES:
        return EDGE_VOICE_MALE
    if gemini_voice in GEMINI_FEMALE_VOICES:
...
def _next_cooldown_eta(job, now):
    """কখন কোনো key healthy হবে — সবচেয়ে কাছে সময়।"""
    pool = job["key_pool"]
...
async def synthesize_segment(
    *,
    text, voice, language="Bengali", gemini_keys,
    edge_pitch="-5Hz", edge_rate="+12%", edge_voice="auto",
    gemini_only=True, key_idx_ref=None, job=None, seg_label="",
    cooldown_sec=300, backup_after_rounds=3, max_rounds=999,
    key_delay_sec=2.0, rpm_limit=15
):
    """
    V3 STRATEGY:
    • Smart pool: healthy keys first, 429 হিট key 5min cooldown
    • Key-এর মধ্যে 2s delay (burst rate-limit এড়ানো)
    ...
    while True:
        ...
        if kinfo is None:
            ...
            _log(f"[{lbl}] 🔴 সব {len(gemini_keys)}টি key cooldown-এ — পরবর্তী {eta}s অপেক্ষা…", "warn")
        ...
        try:
            ...
            fd, tw = tempfile.mkstemp(suffix=".wav")
            os.close(fd)
            ...
            if code == 429:
                ...
                _log(f"[{lbl}] ⚠ {klbl} → 429 → {cooldown_sec}s cooldown (429s={kinfo['total_429']})", "warn")
            else:
                kinfo["total_err"] += 1
...
def _probe_mean_volume_db(path):
    """ffmpeg volumedetect দিয়ে mean volume (dB) extract করে। silent হলে -91dB।"""
    try:
        r = subprocess.run(
...
def apply_ducking(video_path, dubbed_wav, out_path,
                  bg_target_db=-22.0, voice_vol="2dB", bg_floor_db=-50.0,
                  max_bg_boost_db=6.0):
    """
    Enhanced merge_background_and_vocals (zip reference + amplitude check):
    Step 1: video-এর audio mean volume probe করো
    Step 2: যদি audio silent বা floor-এর নিচে (-50dB), bg=0 (mute)
    Step 3: নয়তো bg_target_db (default -22dB)-এ normalize করার জন্য gain হিসাব করো
            (কখনও max_bg_boost_db = +6dB এর বেশী boost করব না — noise amplify এড়ানো)
    Step 4: voice + normalized bg → amix
    """
    mean_db = _probe_mean_volume_db(video_path)
    if mean_db is None or mean_db <= bg_floor_db:
        # Effectively silent original audio — just use voice
        bg_gain = -60.0
    else:
    ...
    fc = (f"[0:a]volume={bg_gain:.2f}dB[bg];"
...
def _run_tts_job(job_id):
    ...
    if do_concat and not job.get("_concat_done"):
        ...
        job["segments"] = [
            {"idx": i, "start": s["start"], "end": s["end"], "text": s["text"], "status": "pending"}
    ...
    try:
        ...
        for i in range(total_segs):
            # CRITICAL FIX: reload job AND re-bind seg via index
            # পুরনো version-এ seg local ref → save_job() reload করার পর
            # status="done" hariye jeto। index দিয়ে fresh job["segments"][i] use করি।
            job = load_job(job_id)
            if not job:
                return
            ...
            if os.path.exists(wav_path) and os.path.getsize(wav_path) > 100:
                ...
                job_log(job, f"[seg{i+1}] ↩️ Resume — file exists")
                save_job(job)
                continue
            ...
            try:
                ...
                # Re-fetch latest job (synthesize_segment may have written to it via _log)
                job = load_job(job_id)
                if not job:
                    return
                seg = job["segments"][i]
        ...
        # Final: re-fetch to compute true done count
        job = load_job(job_id) or job
        job["done"] = sum(1 for s in job["segments"] if s.get("status") == "done")
        job["status"] = "complete"
    ...
    def generate():
        ...
        while True:
            ...
            if done != last_done or status in ("complete", "error", "stopped"):
                ...
                yield (f"event: progress\ndata: {json.dumps({'done': done, 'total': total, 'current': curr, 'seg_text': seg_text, 'status': status}, ensure_ascii=False)}\n\n")
                last_done = done
            ...
            if status in ("complete", "error", "stopped"):
                ...
                # ----- DIAGNOSTIC DUMP (just before video build trigger) -----
                ...
                yield (f"event: log\ndata: {json.dumps({'entries': [{'t': round(time.time(),2), 'msg': _ln, 'lvl': 'info'} for _ln in _diag_lines]}, ensure_ascii=False)}\n\n")

                yield (f"event: done\ndata: {json.dumps({'results': results, 'status': status, 'ok': ok_n, 'total': len(results)}, ensure_ascii=False)}\n\n")
                return
...
def get_platform(url):
    if re.search(r'instagram\.com', url, re.I):
        return 'instagram'
    if re.search(r'tiktok\.com|vm\.tiktok\.com', url, re.I):
        return 'tiktok'
    if re.search(r'kuaishou\.com|v\.kuaishou\.com', url, re.I):
        return 'kuaishou'
...
def start_xray(link=None):
    ...
    try:
        if re.match(r"vmess://", link, re.I):
            d = _decode_vmess(link)
        elif re.match(r"vless://", link, re.I):
            d = _decode_vless(link)
        else:
...
def ytdlp_download(url, out_dir, job_log_fn=None):
    ...
    for strat in strategies:
        ...
        for f in os.listdir(out_dir):
            if not f.startswith("dl_") or f.endswith(".part"):
                continue
    ...
    for attempt in range(4):
        try:
            async with httpx.AsyncClient(timeout=60) as client:
                ...
                if not downloads:
                    raise ValueError("KS-API: no download URL")
...
async def get_ks_video_url_via_graphql(raw_url):
    ...
    if not photo_id:
        raise ValueError(f"Cannot extract photoId from {resolved_url}")
    payload = {
        ...
        try:
            async with httpx.AsyncClient(timeout=180) as client:
                ...
                if resp.status_code in retryable:
                    last_error = ValueError(f"key {idx} rate-limited ({resp.status_code})")
                    continue
                resp.raise_for_status()
                return resp.json()
        except httpx.HTTPStatusError as e:
            ...
            if status in retryable or (status and status >= 500):
                last_error = ValueError(f"key {idx} failed {status}")
                continue
            raise
        except Exception as e:
            last_error = e
...
def transcribe_stream(url, groq_keys_raw, language="zh"):
    ...
    try:
        ...
        if platform == 'kuaishou':
            ...
            yield sse("log", {"msg": "⬇️ Downloading video..."})
            asyncio.run(download_video(video_url, video_path))
        else:
            yield sse("log", {"msg": f"⬇️ Downloading via yt-dlp ({platform})..."})
...
def synthesize():
    ...
    try:
        ...
        if provider == "gemini_audio":
            ...
            if not gemini_key:
                raise ValueError("gemini_key required")
            ...
            os.close(fd_chunk)
            subprocess.run(
                ["ffmpeg", "-y", "-i", orig_audio_path, "-ss", str(start), "-to", str(end), "-c", "copy", chunk_path],
                check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            )
        ...
        os.close(fd)
        subprocess.run(
            ["ffmpeg", "-y", "-i", mp3_path, "-ar", "24000", "-ac", "1", "-sample_fmt", "s16", wav_path],
            check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
        with open(wav_path, "rb") as f:
            wav_bytes = f.read()
...
def dub():
    ...
    # ----- DIAGNOSTIC DUMP at /dub entry point -----
    ...
    for _i, _s in enumerate(segments[:3]):
        _pcm = _s.get("pcm_b64", "")
        print(
            f"[DIAG] seg[{_i}] start={_s.get('start',0):.2f} end={_s.get('end',0):.2f} "
            f"pcm_b64_exists={bool(_pcm)} pcm_b64_length={len(_pcm)}", flush=True
        )
    ...
    try:
        ...
        subprocess.run(
            ["ffmpeg", "-y", "-f", "lavfi", "-i", f"anullsrc=r=24000:cl=mono", "-t", str(duration), base_audio],
            check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
        ...
        for i, seg in enumerate(segments):
            ...
            subprocess.run(
                ["ffmpeg", "-y", "-i", raw_wav, "-af", ",".join(atempo), fit_wav],
                check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            )
        ...
        if seg_files:
            ...
            subprocess.run(
                ["ffmpeg", "-y"] + inputs + ["-filter_complex", ";".join(filter_parts), "-map", "[aout]", mixed_audio],
                check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            )
        else:
...
def setup_config_get():
    cfg = load_config()
    if "VMESS_LINK" in cfg:
        cfg["VMESS_LINK"] = cfg["VMESS_LINK"][:20] + "…"
    return jsonify(cfg)
...
def setup_proxy_test():
    ...
    try:
        r = subprocess.run(
            ["curl", "-x", proxy, "-s", "--max-time", "15", "-o", "/dev/null", "-w", "%{http_code}", "https://api.ipify.org"],
